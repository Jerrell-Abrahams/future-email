...
const ConnectDb = async () => {
  const uri =
    "mongodb+srv://jerrellabrahams50:jerrellabrahams50@cluster0.ooiycdu.mongodb.net/?retryWrites=true&w=majority";
  const options = {
    dbName: "future-email",
  };
  try {
...
app.post("/", async function (req, res) {
  ...
  if (years.length <= 2) {
    ...
    var newDate = new Date(
      currentDate.setMonth(currentDate.getMonth() + parseInt(years))
    );
  } else {
    var newDate = new Date(years);
  }

  if (letterContent === "" || email === "" || newDate === "") {
